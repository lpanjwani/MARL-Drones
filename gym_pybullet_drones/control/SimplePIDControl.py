...
class SimplePIDControl(BaseControl):
    ...
    def __init__(self, drone_model: DroneModel, g: float = 9.8):
        ...
        if self.DRONE_MODEL != DroneModel.HB:
            ...
            exit()
        self.P_COEFF_FOR = np.array([0.1, 0.1, 0.2])
        self.I_COEFF_FOR = np.array([0.0001, 0.0001, 0.0001])
        self.D_COEFF_FOR = np.array([0.3, 0.3, 0.4])
        self.P_COEFF_TOR = np.array([0.3, 0.3, 0.05])
        self.I_COEFF_TOR = np.array([0.0001, 0.0001, 0.0001])
    ...
    def computeControl(
        ...
        thrust, computed_target_rpy, pos_e = self._simplePIDPositionControl(
            control_timestep, cur_pos, cur_quat, target_pos
        )
        rpm = self._simplePIDAttitudeControl(
    ...
    def _simplePIDPositionControl(
        self, control_timestep, cur_pos, cur_quat, target_pos
    ):
    ...
    def _simplePIDAttitudeControl(self, control_timestep, thrust, cur_quat, target_rpy):
        ...
        """
        cur_rpy = p.getEulerFromQuaternion(cur_quat)
        rpy_e = target_rpy - np.array(cur_rpy).reshape(
            3,
        )
        if rpy_e[2] > np.pi:
