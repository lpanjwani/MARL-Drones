...
def run(
    ...
    if (
        ...
        or act == ActionType.ONE_D_DYN
        or act == ActionType.ONE_D_PID
    ):
    ...
    if env_name == "takeoff-aviary-v0":
        train_env = make_vec_env(
            TakeoffAviary, env_kwargs=sa_env_kwargs, n_envs=cpu, seed=0
        )
    if env_name == "hover-aviary-v0":
        train_env = make_vec_env(
            HoverAviary, env_kwargs=sa_env_kwargs, n_envs=cpu, seed=0
        )
    if env_name == "flythrugate-aviary-v0":
        train_env = make_vec_env(
            FlyThruGateAviary, env_kwargs=sa_env_kwargs, n_envs=cpu, seed=0
        )
    if env_name == "tune-aviary-v0":
        train_env = make_vec_env(
            TuneAviary, env_kwargs=sa_env_kwargs, n_envs=cpu, seed=0
        )
    ...
    model.learn(
        total_timesteps=steps,  # int(1e12),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help="Number of training time steps (default: 35000)",
        metavar="",
    )
    parser.add_argument(
        "--output_folder",
